...
class AccountRepository implements AccountRepositoryInterface
{
    ...
    {
        return $this->accountsQuery()->accountTypeIn($types)->count();
    }
    ...
    public function expandWithDoubles(Collection $accounts): Collection
    {
        ...
        foreach ($accounts as $account) {
            $byName = $this->accountsQuery()->where('name', $account->name)->where('id', '!=', $account->id)->first();
            if (null !== $byName) {
                $result->push($account);
                $result->push($byName);
                continue;
            ...
            if (null !== $account->iban) {
                $byIban = $this->accountsQuery()->where('iban', $account->iban)->where('id', '!=', $account->id)->first();
                if (null !== $byIban) {
                    $result->push($account);
                    $result->push($byIban);
                    continue;
    ...
    {
        return $this->accountsQuery()->find($accountId);
    }
    ...
    public function setUser(User $user): void
    {
        ...
        if ($user->isSuperAdmin() && session()->has('active_user_group')) {
            $this->activeUserGroup = session()->get('active_user_group');
        } else {
            $this->activeUserGroup = $user->user_group_id;
        }
    }

    public function accountsQuery()
    {
        return $this->activeUserGroup === 'all' ? Account::query() : $this->user->userGroup->accounts();
    }
